...
<template>
    <AuthenticatedLayout>
        <Head :title="`Detail Jurusan ${major.code} - ${major.name}`" />

        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    <span class="text-gray-400 font-normal">Statistik /</span> Jurusan {{ major.code }} - {{ major.name }}
                </h2>
                <div class="flex items-center gap-3">
                    <!--<a :href="route('reports.major.pdf', major.id)" target="_blank" class="text-sm font-bold bg-red-50 text-red-600 hover:bg-red-100 px-4 py-2 rounded-lg transition-colors flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        Cetak PDF Jurusan
                    </a>-->
                    <Link :href="route('reports.majors')" class="text-sm font-medium text-gray-500 hover:text-indigo-600 transition-colors flex items-center gap-1">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:rounded-2xl border border-gray-100">
                    <div class="p-6 md:px-8 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                        <h3 class="text-lg font-bold text-gray-800">Daftar Anggota jurusan</h3>
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody>
                                <template v-for="(student, index) in filteredStudents" :key="student.id">
                                    <tr class="bg-white border-b hover:bg-indigo-50/30 transition-colors cursor-pointer" @click="toggleStudent(student.id)">
                                ...
                                <tr v-if="filteredStudents.length === 0">
                                    <td colspan="4" class="px-6 py-12 text-center text-gray-500 bg-gray-50/50">
                                        Siswa yang dicari tidak ditemukan dalam jurusan ini.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
